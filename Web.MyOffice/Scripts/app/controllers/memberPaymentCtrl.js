...
controllers.controller('memberPaymentCtrl', ['$scope', '$rootScope', '$routeParams', '$location', 'MemberPaymentService',
    function ($scope, $rootScope, $routeParams, $location, MemberPaymentService) {
        $scope.refresh = function () {
            $scope.memberPayment = MemberPaymentService.get({ id: $routeParams.id, projectId: $routeParams.projectId, currencyId: $routeParams.currencyId }, function () {
            });
        };
        ...
        $scope.delete = function (memberPayment) {
            bootbox.confirm(ASE.L.Delete + ': ' + moment(memberPayment.DateTime).format("L LT") + ". " + memberPayment.Amount + ' ' + $scope.currencyById(memberPayment.Project.RateCurrencyType).ShortName, function (result) {
                if (result) {
