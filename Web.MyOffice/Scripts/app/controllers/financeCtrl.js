...
controllers.controller('financeCtrl', ['$scope', '$rootScope', '$routeParams', 'FinanceService', 'MemberDayReportService', 'pdfService', 'emailService',
    function ($scope, $rootScope, $routeParams, FinanceService, MemberDayReportService, pdfService, emailService) {
        $scope.refresh = function () {
            $scope.project = FinanceService.get({ id: $routeParams.id }, function () {
                ...
                $scope.deleteDR = function (memberDayReport) {
                    bootbox.confirm(ASE.L.Delete + ': ' + moment(memberDayReport.DateTime).format("L LT") + "." + memberDayReport.Description, function (result) {
                        if (result) {
                ...
                $scope.sendPDF = function (project, member) {
                    bootbox.confirm(ASE.L.SendTo + ': ' + member.Member.Email + ' (' + member.MemberTypeS + ', ' + member.Member.FullName + ')', function (result) {
                        if (result) {
