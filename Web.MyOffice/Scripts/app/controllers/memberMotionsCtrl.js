...
controllers.controller('memberMotionsCtrl', ['$scope', '$rootScope', '$routeParams', 'MemberMotionsService', 'pdfService', 'MemberDayReportService', 'MemberMotionsService', 'MemberPaymentService',
    function ($scope, $rootScope, $routeParams, MemberMotionsService, pdfService, MemberDayReportService, MemberMotionsService, MemberPaymentService) {
        ...
        $scope.refresh = function () {
            ...
            $scope.project = MemberMotionsService.query({ projectId: $routeParams.projectId, currencyId: $routeParams.currencyId, dateFrom: $scope.edDateFrom, dateTo: $scope.edDateTo }, function () {
            });
        };
        ...
        $scope.delete = function (motion, type) {
            bootbox.confirm(ASE.L.Delete + ': ' + motion.Description + ', ' + moment(motion.DateTime).format("L LT") + ', ' + ASE.L.Summ + ' ' + motion.Amount, function (result) {
                if (result) {
                    if (type == 1)
